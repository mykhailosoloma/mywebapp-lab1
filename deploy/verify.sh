...
if sudo nginx -t 2>/dev/null; then
    ...
    ok "nginx config is valid (syntax ok)"
fi
...
HEALTH_STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 10 "${BASE_URL}/health/alive")
...
NOT_FOUND_STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 10 "${BASE_URL}/unknown-path")
...
if grep -q "proxy_set_header.*X-Real-IP" /etc/nginx/sites-available/mywebapp 2>/dev/null; then
    ok "nginx sets X-Real-IP header"
else
    fail "nginx does NOT set X-Real-IP header"
fi

if grep -q "proxy_set_header.*X-Forwarded-For" /etc/nginx/sites-available/mywebapp 2>/dev/null; then
...
TASKS_STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 10 -H "Accept: application/json" "${BASE_URL}/tasks")
...
TASKS_RESPONSE=$(curl -s --max-time 10 -H "Accept: application/json" "${BASE_URL}/tasks")
if echo "$TASKS_RESPONSE" | grep -qE '^\['; then
    ok "GET /tasks returns JSON array"
else
    fail "GET /tasks did not return JSON array"
fi

echo ""
echo "════════════════════════════════════════════════"
echo "  Результат верифікації: ${PASS} passed, ${FAIL} failed"
echo "════════════════════════════════════════════════"
